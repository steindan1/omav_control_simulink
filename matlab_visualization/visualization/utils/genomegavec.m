...
function [ obj ] = genomegavec(ax, name, fontsize, width, max_head_size, color)
    ...
    hold(ax,'on');
    % just one arrow in z-direction
    obj{1} = quiver3(0,0,0,0,0,1,0,'Parent',ax);
    hold(ax,'off');

    % Configure quiver frame format properties
    obj{1}.Color = color;
    obj{1}.LineWidth = width;
    obj{1}.MaxHeadSize = max_head_size;
